...
export class BabiesAnalyticsService {
  ...
  async getBabyAnalytics(
    analyticsDto: BabyAnalyticsRequestDto,
  ...
  private async getSleepAnalytics(
    ...
    const averageSleepDurationPerDay =
      totalDays > 0 ? totalSleepDuration / totalDays : 0;

    return {
      totalSleeps,
      totalSleepDuration: Math.round(totalSleepDuration * 100) / 100,
      averageSleepDurationPerDay:
        Math.round(averageSleepDurationPerDay * 100) / 100,
    };
  }
  ...
  private async getFeedAnalytics(
    ...
    const averageFeedingTimePerDay =
      totalDays > 0 ? totalFeedingHours / totalDays : 0;

    // Feed type breakdown
    const feedTypeBreakdown: Record<string, number> = {};
    Object.values(FeedType).forEach((type) => {
      feedTypeBreakdown[type] = feedLogs.filter(
        (log) => log.feedType === type,
      ).length;
    });
    ...
    Object.values(Position).forEach((position) => {
      feedPositionBreakdown[position] = feedLogs.filter(
    ...
    return {
      ...
      averageFeedingTimePerDay:
        Math.round(averageFeedingTimePerDay * 100) / 100,
      feedTypeBreakdown,
  ...
  private async getDiaperAnalytics(
    ...
    const averageDiaperChangesPerDay =
      totalDays > 0 ? totalDiaperChanges / totalDays : 0;

    // Diaper type breakdown
    const diaperTypeBreakdown: Record<string, number> = {};
    Object.values(DiaperType).forEach((type) => {
      diaperTypeBreakdown[type] = diaperLogs.filter(
        (log) => log.diaperType === type,
      ).length;
    });
    ...
    return {
      ...
      averageDiaperChangesPerDay:
        Math.round(averageDiaperChangesPerDay * 100) / 100,
      diaperTypeBreakdown,
    };
  }
}
