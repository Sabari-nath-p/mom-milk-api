...
export class PugAdapter implements TemplateAdapter {
    ...
    compile(mail: any, callback: (err?: any, body?: string) => any, options: MailerOptions): void {
        ...
        const templatePath = join(templateDirectory, `${templateBaseName}${extension}`);

        renderFile(templatePath, { ...context, ...configuredOptions }, (err, html) => {
            if (err) {
                callback(err);
                return;
            }
            ...
            try {
                mail.data.html = this.inlineCssEnabled ? inline(html, this.inlineCssOptions) : html;
                callback();
            } catch (e) {
                callback(e);
            }
