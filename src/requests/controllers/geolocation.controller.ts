...
export class GeolocationController {
  ...
  async getAllZipCodes(
    ...
    return this.geolocationService.findAllZipCodes(
      parseInt(page),
      parseInt(limit),
    );
  }
  ...
  @ApiQuery({
    name: "q",
    description: "Search query (zipcode, place name, or country)",
  })
  @ApiQuery({ name: "page", required: false, description: "Page number" })
  @ApiQuery({ name: "limit", required: false, description: "Items per page" })
  @ApiResponse({
  ...
  async searchZipCodes(
    ...
    return this.geolocationService.searchZipCodes(
      query,
      parseInt(page),
      parseInt(limit),
    );
  ...
  async findNearbyZipCodes(
    ...
    return this.geolocationService.findNearbyZipCodes(
      zipcode,
      parseInt(radius),
    );
  }
  ...
  @ApiOperation({
    summary:
      "Import zipcodes from Excel (.xlsx) file (Admin only) - Clears existing data",
  })
  ...
  @Post("zipcodes/sync")
  @ApiOperation({
    summary: "Sync zipcodes from User table to ZipCode table (Admin only)",
  })
  @ApiResponse({
    status: 200,
    ...
    schema: {
      example: {
        totalUsersChecked: 100,
        uniqueUserZipcodes: 5,
        missingInDb: 2,
        successfullySynced: 1,
        failedToSync: 1,
        failedZipcodes: ["999999"],
      },
    },
  })
  ...
  @ApiResponse({
    status: 200,
    description: "All zipcode data cleared successfully",
  })
  ...
  @ApiResponse({
    status: 200,
    description: "Zipcode statistics retrieved successfully",
  })
