...
export class SleepLogsController {
  ...
  @ApiResponse({
    status: 200,
    description: "List of sleep logs within the date range",
  })
  @ApiResponse({ status: 404, description: "Baby not found" })
  findByBabyIdAndDateRange(
    ...
    return this.sleepLogsService.findByBabyIdAndDateRange(
      babyId,
      startDate,
      endDate,
    );
  ...
  @ApiOperation({
    summary: "Get sleep analytics for a baby within a date range",
  })
  @ApiParam({ name: "babyId", description: "Baby ID" })
  @ApiQuery({
    name: "startDate",
  ...
  @ApiResponse({
    status: 200,
    description: "Sleep analytics including duration calculations",
  })
  @ApiResponse({ status: 404, description: "Baby not found" })
  getSleepAnalytics(
    @Param("babyId", ParseIntPipe) babyId: number,
    ...
    return this.sleepLogsService.getSleepDurationAnalytics(
      babyId,
      startDate,
      endDate,
    );
  ...
  @ApiParam({
    name: "location",
    enum: SleepLocation,
    description: "Sleep location (CRIB, BED, STROLLER, OTHER)",
  })
  @ApiResponse({
    status: 200,
    description: "List of sleep logs with specified location",
  })
  ...
  @ApiResponse({
    status: 200,
    description: "All sleep logs deleted successfully",
  })
  @ApiResponse({ status: 404, description: "Baby not found" })
