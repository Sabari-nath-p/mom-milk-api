...
export class SleepLogsService {
  ...
  async create(createSleepLogDto: CreateSleepLogDto) {
    ...
    if (!baby) {
      throw new NotFoundException(
        `Baby with ID ${createSleepLogDto.babyId} not found`,
      );
    }
  ...
  async findByBabyIdAndDateRange(
    babyId: number,
    startDate: string,
    endDate: string,
  ) {
  ...
  async getSleepDurationAnalytics(
    babyId: number,
    startDate: string,
    endDate: string,
  ) {
    ...
    const totalHours = sleepAnalytics.reduce(
      (sum, log) => sum + log.durationHours,
      0,
    );
    const averageSleepHours =
      sleepAnalytics.length > 0
