...
export class EcomProductsService {
  ...
  async create(userId: number, dto: CreateProductDto) {
    ...
    if (!vendor)
      throw new ForbiddenException(
        "You must have a vendor profile to create products",
      );
    if (!vendor.isActive)
      throw new ForbiddenException("Your vendor account is inactive");
    ...
    return this.prisma.ecomProduct.create({
      data: {
        ...
        variants:
          variants && dto.hasVariants ? { create: variants } : undefined,
      },
  ...
  async addVariant(userId: number, productId: number, dto: CreateVariantDto) {
    const { product } = await this.vendorService.ensureVendorOwnsProduct(
      userId,
      productId,
    );
    if (!product.hasVariants)
      throw new BadRequestException(
        "This product does not support variants. Set hasVariants=true first.",
      );
  ...
  async deleteImage(userId: number, imageId: number) {
    const image = await this.prisma.ecomProductImage.findUnique({
      where: { id: imageId },
    });
