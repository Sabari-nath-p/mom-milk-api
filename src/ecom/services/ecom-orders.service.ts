...
import { PlaceOrderDto, UpdateOrderStatusDto, OrderQueryDto } from '../dto/order.dto';
...
export class EcomOrdersService {
    ...
    async cancelOrder(userId: number, orderId: number) {
        ...
        const cancellableStatuses: EcomOrderStatus[] = [EcomOrderStatus.PENDING, EcomOrderStatus.CONFIRMED];
    ...
    async updateOrderStatus(userId: number, orderId: number, dto: UpdateOrderStatusDto, isAdmin = false) {
        let order: any;

        if (isAdmin) {
            order = await this.prisma.ecomOrder.findUnique({ where: { id: orderId } });
        } else {
