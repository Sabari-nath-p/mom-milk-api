...
export class EcomReviewsController {
    ...
    @Get()
    @ApiOperation({ summary: 'List reviews for a product (verified purchases shown first)' })
    ...
    @ApiBearerAuth()
    @Post()
    @ApiOperation({ summary: 'Leave a review for a product (provide orderId for verified purchase badge)' })
    @ApiParam({ name: 'productId', type: Number })
    createReview(
        @Request() req,
        @Param('productId', ParseIntPipe) productId: number,
