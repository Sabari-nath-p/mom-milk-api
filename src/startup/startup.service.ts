...
export class StartupService implements OnModuleInit {
  ...
  private async importZipCodesOnStartup() {
    ...
    const existingZipCodes = await this.geolocationService.findAllZipCodes(
      1,
      1,
    );
    ...
    if (existingZipCodes.pagination.totalItems > 0) {
      this.logger.log(
        `Zipcode data already exists (${existingZipCodes.pagination.totalItems} records). Skipping import.`,
      );
      return;
    }
    ...
    const excelFiles = fs
      ...
      .filter(
        (file) =>
          ...
          file.toLowerCase().includes("zipcode"),
      );
    ...
    if (csvFiles.length === 0) {
      this.logger.warn(
        "No zipcode files found in data directory. Using sample data.",
      );
  ...
  private async importFile(filePath: string) {
    try {
      ...
      const result =
        await this.geolocationService.importZipCodesFromFile(filePath);

      this.logger.log(
        `Successfully imported ${result.imported} zipcode records`,
      );

      if (result.skipped > 0) {
        this.logger.warn(
          `Import completed with ${result.skipped} records skipped (duplicates or invalid data)`,
        );
      }
    } catch (error) {
      this.logger.error(
        `Failed to import zipcode data from ${path.basename(filePath)}:`,
        error,
      );
    }
  }
