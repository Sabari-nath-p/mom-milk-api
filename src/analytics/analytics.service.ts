...
export class AnalyticsService {
  ...
  async getFeedAnalytics(
    ...
    return {
      ...
      averageFeedTimeMinutes:
        feedLogs.length > 0
          ? Math.round(totalFeedTimeMinutes / feedLogs.length)
          : 0,
      totalAmountMl: Math.round(totalAmountMl * 100) / 100,
      averageAmountMl:
        amountCount > 0
          ? Math.round((totalAmountMl / amountCount) * 100) / 100
          : 0,
      feedMethodCount,
      positionBreakdown,
  ...
  async getDiaperAnalytics(
    ...
    const endDate = filter.endDate
      ...
      : diaperLogs.length > 0
        ? diaperLogs[diaperLogs.length - 1].date
        : new Date();
    ...
    return {
      ...
      averageChangesPerDay:
        Math.round((diaperLogs.length / daysDiff) * 100) / 100,
      diaperTypeBreakdown,
  ...
  async getSleepAnalytics(
    ...
    sleepLogs.forEach((log) => {
      ...
      shortestSessionMinutes = Math.min(
    ...
    return {
      ...
      averageSleepHours:
        sleepLogs.length > 0
          ? Math.round((totalSleepMinutes / 60 / sleepLogs.length) * 100) / 100
          : 0,
      averageSessionDurationMinutes:
        sleepLogs.length > 0
          ? Math.round(totalSleepMinutes / sleepLogs.length)
          : 0,
      longestSleepSessionMinutes:
        longestSessionMinutes === 0 ? 0 : Math.round(longestSessionMinutes),
      shortestSleepSessionMinutes:
        shortestSessionMinutes === Infinity
          ? 0
          : Math.round(shortestSessionMinutes),
      locationBreakdown,
