...
export class ChatService {
    ...
    async getOrCreateSession(userId1: number, userId2: number) {
        // Ensure consistent ordering (parent vs donor doesn't matter for unique constraint)
    ...
    async getUserSessions(userId: number, page: number = 1, limit: number = 20) {
        ...
        const sessionsWithParticipants = await Promise.all(
            sessions.map(async (session) => {
                const otherUserId = session.parentId === userId ? session.donorId : session.parentId;
     ...
     */
    async getSessionMessages(sessionId: number, userId: number, page: number = 1, limit: number = 50) {
    ...
    async markMessagesAsRead(userId: number, messageIds: number[]) {
        ...
        const validMessageIds = messages
            .filter(msg => {
                const isParticipant = msg.session.parentId === userId || msg.session.donorId === userId;
